...
func Dbconnection() (*gorm.DB, *sql.DB, error) {
	...
	lcoreStr := fmt.Sprintf(`%s:%s@tcp(%s:%d)/?charset=utf8mb4&parseTime=True&loc=Local`, lDbDetailsRec.Mysql.User, lDbDetailsRec.Mysql.Password, lDbDetailsRec.Mysql.Server, lDbDetailsRec.Mysql.Port)
